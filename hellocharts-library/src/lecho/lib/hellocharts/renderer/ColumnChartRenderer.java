...
public class ColumnChartRenderer extends AbstractChartRenderer {
     ...
     * 是否绘制成 stack类型的柱状图
     */
    private boolean mIsStackColumn = true;
    ...
    private void calculateMaxViewportForSubcolumns(ColumnChartData data) {
        for (Column column : data.getColumns()) {
            for (SubcolumnValue columnValue : column.getValues()) {
                if (columnValue.getValueTop() >= baseValue && columnValue.getValueTop() > tempMaximumViewport.top) {
                    tempMaximumViewport.top = columnValue.getValueTop();
                }
                if (columnValue.getValueTop() < baseValue && columnValue.getValueTop() < tempMaximumViewport.bottom) {
                    tempMaximumViewport.bottom = columnValue.getValueTop();
                }
            }
        }
    }
    ...
    private void calculateMaxViewportForStacked(ColumnChartData data) {
        for (Column column : data.getColumns()) {
            ...
            for (SubcolumnValue columnValue : column.getValues()) {
                if (columnValue.getValueTop() >= baseValue) {
                    sumPositive += columnValue.getValueTop();
                } else {
                    sumNegative += columnValue.getValueTop();
                }
            }
    ...
    private void processColumnForSubcolumns(Canvas canvas, Column column, float columnWidth, int columnIndex,
        ...
        for (SubcolumnValue columnValue : column.getValues()) {
            ...
            final float rawY = computator.computeRawY(columnValue.getValueTop());
            if (mIsStackColumn) {
                final float rawYBottom = computator.computeRawY(columnValue.getValueBottom());
                calculateRectToDraw(columnValue, subcolumnRawX, subcolumnRawX + subcolumnWidth, baseRawY, rawY, rawYBottom);
    ...
    private void processColumnForStacked(Canvas canvas, Column column, float columnWidth, int columnIndex, int mode) {
        ...
        for (SubcolumnValue columnValue : column.getValues()) {
            ...
            if (columnValue.getValueTop() >= baseValue) {
                ...
                mostPositiveValue += columnValue.getValueTop();
            } else {
                subcolumnBaseValue = mostNegativeValue;
                mostNegativeValue += columnValue.getValueTop();
            }
            final float rawBaseY = computator.computeRawY(subcolumnBaseValue);
            final float rawY = computator.computeRawY(subcolumnBaseValue + columnValue.getValueTop());
    ...
    private void calculateRectToDraw(SubcolumnValue columnValue, float left, float right, float rawBaseY, float rawY, float rawYBottom) {
        // Calculate rect that will be drawn as column, subcolumn or label background.
        drawRect.left = left;
        drawRect.right = right;
        if (columnValue.getValueTop() >= baseValue) {
...
//            drawRect.bottom = rawY;
    ...
    public boolean ismIsStackColumn() {
        return mIsStackColumn;
    }

    public void setmIsStackColumn(boolean mIsStackColumn) {
        this.mIsStackColumn = mIsStackColumn;
    }
}
